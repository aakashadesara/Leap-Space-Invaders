...
var skyrim = new Audio('sounds/skyrim.mp3'); //
var countdown = new Audio('sounds/countdown.mp3'); //
var gangnam = new Audio('sounds/gangnam.mp3'); //


var array = [ bit, chocolate, fox, jackson, dawn, skyrim, countdown, gangnam];

var audio = array[Math.floor(Math.random() * array.length + 1)];

>>>>>>> d59f4388db9f89cded6aa6fc33c29463b06ac358
function Game() {
...
PlayState.prototype.update = function(game, dt) {
    ...
    var leapToScene = function( frame , position , size ){

        var iBox = frame.interactionBox;
        var left = iBox.center[0] - iBox.size[0]/2;

        var newPosition = ( position - left ) / iBox.size[0];
    ...
    //this.ship.x = controller.frame().pointables.leftmost.tipPosition.x;
    if(frame.hands.length > 0){
        var pos = frame.hands[0].palmPosition[0];

        this.ship.x = leapToScene( frame , pos , 600 );
        console.log( this.ship.x )
        //this.ship.x = frame.pointables[0].tipPosition[0];
...
PlayState.prototype.draw = function(game, dt, ctx) {
    ...
    var monster11 = document.getElementById("monster11");
    var monster12 = document.getElementById("monster12");
    var monster21 = document.getElementById("monster21");
    var monster22 = document.getElementById("monster22");
    var ship = document.getElementById("ship");


    ctx.drawImage(ship,0,0, 200,200, this.ship.x - (this.ship.width / 2), this.ship.y - (this.ship.height / 2), 40, 40);



    //console.log( img );
    //ctx.drawImage(img,0,0, 150,120, (this.ship.width / 2), this.ship.y - (this.ship.height / 2), this.ship.width, this.ship.height);
    ...
    for(var i=0; i<this.invaders.length; i++) {
        ...
        ctx.drawImage(invader.img,0,0, 250,250, invader.x - invader.width/2, invader.y - invader.height/2, invader.width, invader.height);
    ...
    for(var i=0; i<this.bombs.length; i++) {
        var bomb = this.bombs[i];
        ctx.fillRect(bomb.x - 2, bomb.y - 2, 6, 16);
    }
    ...
    for(var i=0; i<this.rockets.length; i++) {
        var rocket = this.rockets[i];
        ctx.fillRect(rocket.x, rocket.y - 2, 6, 12);
    }
...
function Invader(x, y, rank, file, type) {
    ...
    if(rank % 2 == 0){
        this.img = document.getElementById('monster11');
    } else {    
        this.img = document.getElementById('monster21');
    }    
}
